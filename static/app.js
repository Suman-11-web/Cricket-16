...
async function fetchTables() {
    try {
        ...
        pts.forEach((r, i) => ptsHTML += `<tr><td style="color:var(--text-muted);">${i+1}</td><td style="text-align:left; font-weight:800; color:white;">${r.team}</td><td>${r.p}</td><td>${r.w}</td><td>${r.l}</td><td style="color:var(--neon-green)">${r.nrr}</td><td style="color:white; font-weight:900; font-size:1.1rem;">${r.pts}</td></tr>`);
